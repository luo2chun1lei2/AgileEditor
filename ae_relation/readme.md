...
1. 关系(relation)是元素之间的关系，实例必须关联两个以上对象。需要强调的是，关系也是元素。
  ...
  1. 类型（type）是两个元素之间的关系。
   ...
   1. 为了方便使用。针对添加类型，元素可以按照类型衍生出来。
    ...
    类到元素是衍生，元素到类是继承。
1. 基本元素(basic Element)
  1. 这里的元素先排除 Relation，这个用线表示。
  1. 其他的元素可以是 类型、实例、功能。【类型和实例并非本质！虽然在编程上类型和实例是不变的】
  1. 类型是说：这个元素是其他元素的关系和描述，自己本身是不能运行的。不过它可以用于表述此类型的实例都是如何运行的。
  1. 实例是说：类型下的具体例子，如果没有特殊的地方，那么就不需要特殊的说明。比如统合所有名字的管理实例，
    这里强调所有地方都必须用这一个。
  1. 功能：这个是纯粹的运行逻辑，可能需要外部的实例。
1. 基本的关系(basic relation)
  1. 只有在一个规定的系统中，才能说关系。目前比较常用的我提出：类型、组、依赖。
  1. 类型是说，一个对象是另一个对象的描述或抽象说明。在程序中就是 类和实例。
  1. 组是是说，一个对象拥有另外一个对象，所以当此对象生成或销毁时，另外一个对象也同时生成和销毁。
   比较常见的是实例和成员的关系。但是在实际的编程中，实例的成员有可能是另外一个独立的对象，并不随着实例生成和销毁。
  1. 依赖是说，一个对象需要另外一个对象，可能是调用、使用等更加细分的关系，但是总之是依赖。
  “组”和“依赖”两个在编程中比较含糊，虽然实际编程中却很重要，因为涉及到资源的申请和释放，或者状态的保存等等。
  我的希望是，之后延伸出来的关系，都是在这三个关系的基础上衍生出来的（目前没有哲学或者说数学的根据），
  需要在实践中不断的尝试和微调，当然我也希望能够从某种理论上获取正确的分析。
1. 基本关系 -> 基本表现，有如下几种设计方案：
  1. 衍生关系 -> 基本关系 -> 基本表现，好处是无论什么衍生关系，都可以表现出来。
  1. 衍生关系 -> 衍生表现 -> 基本表现，好处是可以直接对应复杂的output
...
我设想了这么一个系统，它可以描述各种对象之间的关系，然后
1. 根据对象、关系的进行查询、重新整理，找到解决问题的方法。
2. 将描述中的关系设定特定的表现方式，然后表现出来。比如，汽车有车轮，“有”就设置上表现的标签“包含”，这样就显示出来。或者设置成UML的表现标签，这样也能用UML显示出来。
3. 让系统运行起来，赋予关系以某种计算或运行的意义，这样可以静态的看到结果，或者动态的看到结果。
