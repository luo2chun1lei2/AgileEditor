...
9，全新的结构：
	...
	3，模型通论：
		...
		1，系统中任何一个研究目标都是一个对象，用小写表示，后面可以加类型，比如 c1:C1，表示 c1 对象是 C1 类型的，":C1" 表示C1类型的任一实例。
			复数表示集合，单数表示其中之一。
		2，{} 表示有名字的对象集合，允许重复，[]表示无序集合，不允许重复，<> 表示有顺序的序列，可重复。
			() 表示相关性，比如 f(x)，表示x和f是相关的。如果是 (x)，无特别的意思。
			所有括号根据嵌套的层次也表示优先级，优先考虑括号内的对象，然后再考虑括号外的。
			if 表示当什么发生，或者什么为真时，比如 "if x==5 >> f"，表示 “x==5” 成立时，会引发 f 动作。 if x >> f，表示“只要x存在，就有动作f”。
			<< 表示"导致、变成"，是单向的动作，不是关系，用来代表“当发生了什么后，导致了什么结果”，上面也可以表示为 “if x==5 >> f”
			<= 单向赋值，单向关系，比如 x <= 5，表示“x变成了5”，或者“5赋值给x”，反过来不能说 5 就是 x。
			= 就是等同关系，表示系统一定要符合的关系，可以用 = 的一边代表另外一边。比如"a=b+5"，当b是4时，a必须是9,而a变成1，b一定要是-4。
				所以和“<=”不同，如果“a<=b and a=>b”，那么就是“a=b”。但是程序语言中大部分的“=”都是赋值的意思，并不维护两者之间一致的关系。
		3，逻辑符号“==” 和 “!=”，“>”等，表示“判断是否成立”，比如 “a == 5”，表示需要判断“a是否为5”，也许成立，也许不成立。
			if 表示“当成立”或“假如成立”，是分支条件语句。
			用“assert( a == true )” 表示“一定要xxx成立”。
		4，动作 action 用一个函数来表示，比如 y = f(x)，x 是引入的参数，y是导出的结果，x和y都可以是任何对象。
			y 为什么接 “=”，是因为对于 action 来说，y就等同于 f 导出的结果。
			比如 if x==5 >> y = foo(x)，表示当x是5时会引发foo动作，导入f的参数是x，而导出的结果等同于y。
			f = < y <= x >，表示f的具体动作是“将y从任何值变成现在x的值”，括号中可以写任意多和任意的语句，
				括号是"[]"，表示可并行，如果是“<>”，表示必须按照顺序进行。
				可以表示为 if x==5 >> [y <= x]，那么动作就是隐含名字。
		5，每个对象可以具有任意的状态，比如 c1.states = [none, unregistered, registered]
		...
		6，每个动作有两个方面，一个是实际会做什么，从此可以推断出系统真实的运行情况，另外一个是想做什么，
			这个用于诊断系统的结构是否真的符合要求。这两个一定要分明。
			f = <f1, f2, f3> 是描述实际怎么实现， f >> a <= 3，描述的是期望(f想把a变成3，通过<f1,f2,f3>步骤，但是否可行，没有验证过)。
		7，评价
			u(s) 是针对系统s的评价函数，多个u函数必须满足一定的条件。比如 u1 代表系统反应时间，u1(cn.action) < 200ms，
			表示“任何组件实例的动作反应时间都必须小于200ms”。条件用 if u1 < 200ms。
			系统需要达成的功能: requires(s) = [cond1 and cond2 and cond3]，是 s 必须满足的条件集合。
			需求不一定是静态的，也可以是动态的，就是在输入一些动作序列后改变。
		8，系统是否可以完成功能，是由其结构决定的。S = [f1, f2, f3, f4]
			注意这个是静态的，是系统运行的指导。
		9，系统建立后，会存在内部的运行序列，以及外部的事件序列，都可以用动作序列来表示
			SEQ = <f1, f2, f3, ...>
			当seq影响S时，会产生一个系统队列
			S = <s1, s2, s3, ...>，其中 f1 >> s <= s1，f2 >> s<=s2，以此类推。
			无论这个队列任何元素，都应该满足系统的要求： requires(s) = True。
		10，对象的动作的结果由子动作来实现，那么也可以根据子动作来分析是否满足了需要。
			<f1> >> [s <= s1 and assert(requires(s1) == True)]，对于任意f序列，都要s变成s1，且s1符合条件requires。
	...
	5，验证模型：要生成可能的序列 seq1 = <c1<=C1.new, c1.reg, c1.unreg, ...>，然后看结果是否符合要求。
		1，假定有 c:C.do1 >> [assert(c:Menu.know(c1) == True)]，一个组件如果有do1动作，此动作会导致所有Menu类型的实例都知道 c1。
			动作实现 c:C.do1 = [ c:Menu.add(c1) ]，那么分析动作实现是否可以达到目的。
			而 c:Menu.add(c1) >> [assert(c:Menu.know(c1) == True)]，添加动作会导致 know(c1)是真，不是赋值，而是内部动作导致know返回真。
			根据推理 do1 -> add -> know，所以是正确的。
		2，不过 c:Menu可以unregister，然后再 create，register 一个 c:Menu 时，则 assert(c:Menu.know(c1) == False)。
			所以此设计错误。
