...
先为了能够程序运行不出现错误（允许无法完成正常的功能），可以修改svc为：“请求对应的组件，如果组件没有注册，那么就不调用，返回错误”。

这里依据的想法是：如果此组件没有注册，那么当然无法完成对应的功能，那么就出错。如果之后组件又注册上了，就可以调用了。如果是这样的设计，那么整个序列任意组合都不会出现错误，就不用论证了。

##例子5

上面的例子中，虽然看起来没有问题，不过在实际的程序中，会出现另外一种常见情况，需要某些组件必须调用此组件服务一次。比如说：有一个menu组件，当一个编辑组件加入系统时，它应该在menu中加入用户可以直接使用的选项和功能，这样用户可以点击。但是menu在编辑组件之后加入，或者中间使用中，menu组件崩溃后再添加，那么之前编辑组件在其中注册项目都没有了。对于这种情况怎么设计去解决？

要求：

1. 组件允许任意注册和卸载，没有先后顺序。
2. 组件需要的功能，如果不能在组件需要时一定有，那么应该在准备好后，就通知此组件使用。

设计：

1. 上面讨论时，发现有两种请求，一种是调用时现发现没有注册，完全可以返回错误，等到后面**需要时**再调用就行了，比如没有发现合适的组件打开指定的文件，那么就返回错误，等到再次需要打开时，再检查是否有，如果不再打开。即使注册了此类文件的编辑工具，也没有必要马上打开这个文件。
2. 但是另外一种，当发现有效组件时，需要立刻调用。实际例子就是上面的menu组件。
3. 上面两种情况，我试图用“是否具有状态”来区分，但是打开一个文件也是需要状态的。是否要同步sync。


点滴记录：
1. 2018/3/30 gtk的editor虽然可以编辑markdown，但是无法预览，可以用 python 将markdown文件转换成html，然后用gtk显示html，这样就行了。
2. 2018/4/13 AE的逻辑论证被拖延下来，需要再接再厉，另外之前最重要的基于组件和关系的思考，现在还是没有实现，不要害怕复杂的系统，
  需要考虑的是怎么实现系统，简单设计固然好，无法简单的设计，就需要复杂的结构，不要畏惧困难。
