...
分阶段实现
	...
	第二阶段：和当前项目结合的编辑器
		...
		3, 执行命令
			- 可以在SHELL中执行命令
			- 可以将选中的代码显示在剪贴板中，编辑后，再贴回（和vx结合使用）
...
2，在此主体结构上，添加功能模块。功能模块之间保持彼此之间的独立性。
2.1 模块之前不可见，而是通过发送消息的方式来处理。
2.2 画面和
