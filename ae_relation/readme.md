...
新任务列表：内部模型和外部应用是分开的，当然他们之间也是有配合的。
内部模型：
L1：最基本的element和relation
  1. 最基本的Element 和 Relation：基本属性，保存和恢复snap，脚本生成。
  2. 容纳Element的Model，可以列举和检索，以及通过执行script生成，或者snap来恢复。

L2：实现组合Element。
  1. 组合多个Element形成一个Element。类似集合、数组等概念。
  2. Model建立组合检索功能。
  3. 加入Graph的显示功能。

L3：实现继承或者模板。
  1. 实现”Kind“，可以通过kind来批量生成element。
  2. Model可以进行分析，就是在模型中，按照指定的“条件”得到“答案”。
  
L4: 实现外部程序也用关系描述
  1. 关系可以运行。
  2. 建立可以映射到运行程序模块的元素和关系。
  
应用
L1: 可以启动Model，有简单的输入和输出模块（都是文本）。
L2: App执行脚本，以及交互执行命令。
L3: 可以生成图形的输出，已经图形输入。
L4: 有多种输入或输出。
L5: 可以有多个Model同时处理。
