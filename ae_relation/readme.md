...
4. 如果想建立统一的关系后，用任意UML图显示，那么就先要建立统一的UML关系。
  ...
  -4. 实现新的Input，只需要简单的控制台，不需要 prompt_toolkit。-但是家里的控制台无法使用这些，不知道为什么？
  -5. 建立简单对象之间的关系。（不是UML）-
  6. 允许使用简单的语句就可以实现 关系的描述，包括UML的。现在的语法太复杂和难以描写。
  7. 模块越来愈多，如何组合，谁和谁组合，以及他们之间的一致接口很重要。
   组合方式用“脚本”来描述如何？而不是通过复杂的命令行参数，这个会导致复杂程序命令说明。
   目前Model和Parser、Executor、Output是捆绑在一起的，如果无法从一致的语法进行扩展，那么只能是一套系统就是一套。
  8. 高级的模型可以用简单的模型通过扩展来实现，这个扩展开始可以是一个python的类，之后可能是通过脚本来扩展。
